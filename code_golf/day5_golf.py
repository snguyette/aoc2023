from aocd import get_data; lines = get_data(day=5, year=2023).split("\n") + ['']
seeds = [int(seed) for seed in lines[0].split(': ')[1].split(' ')]; map_list = []
        ...
        for i, seed in enumerate(seeds):
            for map_dict in map_list:
                if map_dict['src_start'] <= seed <= map_dict['src_end']:
                    seeds[i] = map_dict['dst_start'] + (seed - map_dict['src_start']); break
            ...
            for m in map_list:
                if r[0] <= m['src_end'] and r[1] >= m['src_start']:
                    ...
                    ranges[i] = (
                        ...
                        m['dst_start'] + min(r[1], m['src_end']) - m['src_start']); break
        map_list = []
